...
import type { NotesSearchFilterOptions } from "./notes"

const useNotesInfQuery = (
  id: number,
  filter: NotesSearchFilterOptions,
  max?: number
) => useInfiniteQuery(notesInfQueryOptions(id, filter, max))

const notesInfQueryOptions = (
  id: number,
  filter: NotesSearchFilterOptions,
  max?: number
) => {
  return {
    ...
    queryFn: async ({ pageParam }: { pageParam: number }) =>
      await notesInfQuery(id, pageParam, filter, max),
    initialPageParam: 0,
...
export { useNotesInfQuery, notesInfQueryOptions, notesInfQuery }
